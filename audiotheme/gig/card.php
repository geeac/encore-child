...
<dl id="post-<?php the_ID(); ?>" <?php 

if ( get_audiotheme_gig_time('c') < current_time( 'mysql' ) ) 
post_class( array( 'gig-card', 'vevent', 'old' ) );
else post_class( array( 'gig-card', 'vevent' ) );
 ...
 ?> itemscope itemtype="http://schema.org/MusicEvent">
	...
	<?php if ( audiotheme_gig_has_venue() ) : ?>
		...
		if ( ! is_singular( 'audiotheme_gig' ) ) :
			the_audiotheme_gig_description(
				'<dt class="gig-description screen-reader-text">' . __( 'Note', 'encore' ) . '</dt><dd class="gig-description" itemprop="description">',
				'</dd>'
			);
		endif;
		?>
	...
	<?php else : ?>
		...
		<dd class="gig-venue no-gig-venue">
			...
			<?php edit_post_link( __( 'Edit Gig', 'encore' ) ); ?>
		</dd>
